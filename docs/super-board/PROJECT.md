...
- **Never invent or estimate dates, deadlines, or pass rates.** Every value in `src/data/ap-2027.json` must come from College Board's published pages (AP calendar, late-testing calendar, score distribution report). Anything College Board does not publish is **omitted**, and the surfaces render it as the not-published dash (`NotPublishedDash`). There is exactly one unpublished state: issue #84 retired the literal `"pending"` after re-verifying all 33 of its values against the live pages and finding none of them published, and `ap-2027.test.ts` now fails if the string reappears anywhere in the dataset. A capture that cannot reach a page must fail loudly or block the swap — never invent a third state to hide in.
