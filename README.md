...
`GOOGLE_SITE_VERIFICATION` and `BING_SITE_VERIFICATION` are read at build time and rendered as the corresponding `<meta>` verification tags (`google-site-verification` / `msvalidate.01`) for Google Search Console and Bing Webmaster Tools. Set them where the production build runs (on Vercel: Project → Settings → Environment Variables) and redeploy. Unset — the default everywhere — no tag renders, and no token is ever committed to this repo.
